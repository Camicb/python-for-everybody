...
#Actual data: http://py4e-data.dr-chuck.net/comments_884365.xml (Sum ends
#with 19)
...
url = 'http://py4e-data.dr-chuck.net/comments_884365.xml'
uh = urllib.request.urlopen(url, context=ctx)

link = uh.read()
tree = ET.fromstring(link) #create a tree from xml
counts = tree.findall('.//count')# .// means: on 0 or more levels from current level
total=0
for item in counts:
    x=int(item.text)
    total=total+x
